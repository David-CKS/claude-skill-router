...
DEFAULT_SETTINGS = {
    ...
    "gate_grace_ttl_seconds": 300,  # 5 min ventana para el grace period (C2)
    # Context Injection (17-may-2026) — inyecta SKILL.md completo en additionalContext
    # para eliminar la necesidad de que el LLM invoque la tool Skill manualmente.
    "context_injection_enabled": True,
    "context_injection_max_skills": 2,
    "context_injection_max_chars_per_skill": 3000,
    # Embeddings fallback (17-may-2026 coordinator paso 2/5) — semantic search via FAISS
    # cuando keyword/Gemini confidence < threshold. Standalone available, integración
    # con process_user_prompt diferida (causaba regresión clusters keyword-evidentes).
...
def load_clusters_config(cwd: str | None = None) -> tuple[dict, dict, dict]:
    ...
    if CLUSTERS_YAML.exists() and yaml is not None:
        try:
            data = yaml.safe_load(CLUSTERS_YAML.read_text()) or {}
    ...
    if local_path and yaml is not None:
        ...
        try:
            ldata = yaml.safe_load(local_path.read_text()) or {}
